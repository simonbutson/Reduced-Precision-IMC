# 1D Implicit Monte Carlo - Half Precision Su Olson Problem
# Simon Butson
...
dt = Float16(inputs[2,2]) # Timestep length
width = Float16(inputs[3,2]) # Slab width
dx = Float16(inputs[4,2]) # Spatial cell size
timesteps = Float16(inputs[5,2]) # Number of timesteps
n_input = Int16(inputs[6,2]) # Starting particles input
sigma_s = Float16(inputs[7,2]) # Scatter cross-section
sigma_a = Float16(inputs[8,2]) # Absorption cross-section
x_0 = Float16(inputs[9,2]) # Source width
tau_0 = Float16(inputs[10,2]) # Source active time
epsilon = Float16(inputs[11,2]) # Epsilon parameter
T_init = Float16(inputs[12,2]) # Initial slab temperature

tmax = timesteps * dt # Maximum time
shakes = Float16(dt/(1e-8)) # Timesteps in shakes (1e-8)

Ncells = Int16(width/dx) # Number of cells

sourceCells = Int16(x_0/dx) # Number of cells with source

particleVars = 8 # Particle array variables
 
n_census = Int16(0) # Number of particles stored in census
#n_max = 50000000 #
...
particles = Vector{Vector{Float16}}()
mesh_tempsaved = Vector{Vector{Float16}}()
radenergy_saved = Vector{Vector{Float16}}()
materialenergy_saved = Vector{Vector{Float16}}()

mesh_tempinit = T_init .* ones(Float16, Ncells) # Initial temperature (keV)
mesh_temp = copy(mesh_tempinit) # Temperature (keV)
mesh_fleck = zeros(Float16, Ncells) # Fleck factor
mesh_beta = zeros(Float16, Ncells) # Beta factor
mesh_sigma = zeros(Float16, Ncells) # Planck mean opacity
mesh_energydep = zeros(Float16, Ncells) # Deposited energy in a timestep
material_energy = zeros(Float16, Ncells) # Material energy density
radenergydens = zeros(Float16, Ncells) # Radiation energy density

source_energies = Vector{Float16}()
energy_increases = Vector{Float16}()
lost_energy = Vector{Float16}()
stored_energy = Vector{Float16}()
...
alpha = Float16(4 * phys_a / epsilon)

scale = Float16(100)

#phys_invh = 1/phys_h
D = zeros(Float16, Ncells)

# Material values
T0 = Float16(1.0) # 
#bee = 4.0 * (15 / pi^4) * phys_a * phys_c * (1 - scatter_frac) * gamma * dt / (scatter_frac)
bee = zeros(Float16, Ncells)
#bee = 1.0
...
function update()
    ...
    #mesh_sigma[:] = (15 / pi^4) * gamma .* mesh_temp[:].^(-3)
    
    mesh_sigma[:] = sigma_a * ones(Float16, Ncells)

    # Calculate beta (non-linearity) factor
    #mesh_beta[:] = 4.0 * phys_a .* (mesh_temp[:].^3) / bee

    mesh_beta[:] = ones(Float16, Ncells) # Result simplifies to 1 so no need to calculate it

    #mesh_beta[:] = 4.0 * phys_a .* (mesh_temp[:].^3)

    # Calculate Fleck factor
    mesh_fleck[:] = 1.0 ./ (1.0 .+ alpha * mesh_beta[:] * phys_c * dt .* mesh_sigma[:]) # Include absorption cross-section
...
function sourcing(t)    
    ...
    sources = zeros(Float16, Ncells)
    sources[1:sourceCells+1] .= 1.0

    # Check if source is still active
    if t <= tau_0
        e_body = scale*(sigma_a * mesh_fleck[:] .* phys_c * dx * dt * phys_a .* (mesh_temp[:].^4)) .+ scale*(sources[:] * dt * dx)
    else
        e_body = scale*(sigma_a * mesh_fleck[:] .* phys_c * dx * dt * phys_a .* (mesh_temp[:].^4))
    end
    ...
    n_sourcing = n_input # Just use the n_input value rather than calculating n_max which will exceed max(Int16) = 65504 in reasonably sized Calculations
                         # As long as no more than 65504 particles are in each cell, there is no need for batching
    ...
    n_body = zeros(Int16, Ncells)

    # Fleck & Cummings Deterministic Sourcing

    for cellindex in 1:Ncells
        if mesh_temp[cellindex] > 0 || cellindex <= sourceCells+1 && n_sourcing > 0
    ...
    for cellindex in 1:Ncells
        ...
        nrg = e_body[cellindex] / Float16(n_body[cellindex])
        startnrg = nrg

        for j in 1:n_body[cellindex]
            origin = cellindex
            #xpos = mesh_nodes[cellindex] + dx * rand()
            xpos = dx * rand()
            mu = Float16(1.0) - 2 * rand(Float16)
            while mu == 0.0
                mu = Float16(1.0) - 2 * rand(Float16) # Check if mu = 0 and rerun if it is
            end
            spawntime = dt * rand()
            frq = Float16(1.0) # Frequency not used so set to arbitrary value
    ...
    #print("The number of particles coming from the fixed source is ", n_source, "\n")
    print("The number of particles radiated is ", sum(n_body), "\n")

    # print("The number of particles after sourcing is ", length(particles), "\n")
    # check_energy = 0
    # for ii in 1:length(particles)
    #     check_energy += particles[ii][7]
    # end
    # push!(stored_energy, check_energy)
    # print("The energy in the stored particles is ", check_energy, "\n")
end
...
function MC(t)
    ...
    mesh_energydep[:] = zeros(Float16, Ncells)
    absorbed_particles = Float16(0)
    ...
    for particle in 1:length(particles)
        ...
        minenergy = Float16(0.01 * startenergy)
        #D[:] = gamma ./ ((mesh_temp[:].^T_power) * phys_h^3)
        #D[:] = gamma ./ ((mesh_temp[:].^T_power))
        ...
        while true
            ...
            dist_col = abs(randexp(Float16)) / (sigma_s + (1-mesh_fleck[cellindex])*sigma_a) # Effective scattering includes isotropic term
            # Census distance
            ...
            # Collision (i.e. absorption, but treated as pseudo-scattering)
                ...
                freq = mesh_temp[cellindex] * randexp(Float16)
                mu = Float16(1.0) - 2 * rand(Float16)
                while mu == 0.0
                    mu = Float16(1.0) - 2 * rand(Float16)
                end
...
function tally(t)
    ...
    radenergydens = zeros(Float16, Ncells)
    radenergydens[:] = phys_a * mesh_temp[:].^4 # keV/cm^3

    # input_energy = sum(radenergydens)
    # print("The input energy before is: ", input_energy, "\n")

    sourceenergy = zeros(Float16, Ncells)
    sourceenergy[1:sourceCells+1] .= 1.0
    
    # Temperature increase
    nrg_inc = zeros(Float16, Ncells)

    nrg_inc[:] = (mesh_energydep[:]/(dx*scale)) .- (sigma_a * mesh_fleck[:] .* radenergydens[:] * phys_c * dt)
    ...
    radnrg = zeros(Float16, Ncells)

    # # Pairwise summation method for radnrg - More accurate but much slower
    # for jj in 1:Ncells
    #     tempradnrg = Vector{Float16}()
    #     for ii in 1:length(particles)
    #         radindex = Int(particles[ii][3])
    #         if radindex == jj
    #             push!(tempradnrg, particles[ii][7]/(dx*scale)) 
    #         end
    #     end
    #     radnrg[jj] = sum(tempradnrg)
    # end

    # Naive summation - Less accurate but still gives good results 
    for ii in 1:length(particles)
        radindex = Int(particles[ii][3])
        radnrg[radindex] += particles[ii][7]/(dx*scale) 
    end
...
function main()
    t = Float16(0.0) # Start time
    timestep = Int16(0)
    print("Starting IMC Calculations \n")
    while timestep <= timesteps
        print("Simulating time t = ", t, ", timestep = ", timestep, "\n") 
...
plot(mesh_centers, radenergy_saved[Int(timesteps),:], marker="o", title="Half Precision IMC Results for dt = "*string(dt)*"", label="tau = "*string(round(phys_c * dt * timesteps, sigdigits=2))*" IMC", size=(600,600), margin=5Plots.mm)
...
savefig("Radiation Energy Density Half.png")
               
writedlm("RadEnergyResultsHalf.csv", radenergy_saved, ',')
               
# Material energy density
               
plot(mesh_centers, materialenergy_saved[Int(timesteps),:], marker="o", xscale=:log10, yscale=:log10, minorgrid=true, title="Half Precision IMC Results for dt = "*string(dt)*"", label="tau = "*string(round(phys_c * dt * timesteps, sigdigits=2))*" IMC", size=(600,600), margin=5Plots.mm)
...
savefig("Material Energy Density Half.png")
               
writedlm("MaterialEnergyResultsHalf.csv", materialenergy_saved, ',')